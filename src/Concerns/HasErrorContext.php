...
trait HasErrorContext
{
    ...
    /**
     * Breadcrumb notes for debugging context.
     *
     * @var array<int, string>
     */
    ...
    /**
     * Add a breadcrumb note to the exception.
     *
     * Notes provide a chronological trail of context as the exception
     * propagates through the application. Each note adds a layer of
     * debugging information about what was happening when the error occurred.
     * Inspired by Python's exception notes.
     *
     * @param string $note Descriptive note to add
     *
     * @example Add execution context notes
     * ```php
     * $exception->addNote('Processing payment for order #123')
     *           ->addNote('Using Stripe gateway')
     *           ->addNote('Customer retry attempt #2');
     * ```
     * @example Add environmental context
     * ```php
     * $exception->addNote("User ID: {$user->id}")
     *           ->addNote("Session active for {$duration} seconds")
     *           ->addNote("Request from mobile app");
     * ```
     */
     ...
     *
     * @param array<int, string> $notes Array of notes to add
     *
     * @example Add multiple context notes
     * ```php
     * $exception->addNotes([
     ...
     * ```
     */
    public function addNotes(array $notes): static
    {
    ...
    /**
     * Get all notes attached to this exception.
     *
     * @return array<int, string>
     *
     * @example Display notes in log
     * ```php
     * foreach ($exception->getNotes() as $note) {
     *     logger()->debug($note);
     * }
     * ```
     */
    ...
    /**
     * Check if exception has any notes.
     */
    ...
    /**
     * Get formatted notes as a string.
     *
     * @example Log formatted notes
     * ```php
     * logger()->error($exception->getFormattedNotes());
     * ```
     */
