...
describe('findFirst', function (): void {
    test('finds first matching exception in chain', function (): void {
        ...
        expect($top->findFirst(InvalidArgumentException::class))->toBe($middle)
            ->and($top->findFirst(RuntimeException::class))->toBe($root);
    });
    ...
    test('searches through deep chains', function (): void {
        ...
        expect($top->findFirst(RuntimeException::class))->toBe($level3);
    });
});

describe('findAll', function (): void {
    test('finds all matching exceptions in chain', function (): void {
        $runtime1 = new RuntimeException('First');
        $runtime2 = new RuntimeException('Second', 0, $runtime1);
        $top = TestDomainException::withMessage('Top')->wrap($runtime2);

        $results = $top->findAll(RuntimeException::class);

        expect($results)->toHaveCount(2)
            ->and($results[0])->toBe($runtime2)
            ->and($results[1])->toBe($runtime1);
    });
